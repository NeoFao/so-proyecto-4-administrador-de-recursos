...
| Suite de pruebas | 26 pruebas, la misma en ambas plataformas |
...
| M2 | Suite de 26 pruebas con arnés propio | `results/pruebas_windows.txt` | 26/26 |
| M3 | **La misma suite ejecutada en Linux** | `results/compatibilidad_linux.txt` | 26/26 |
...
| M7 | **La carga del RE-2 se comprueba antes y después de medir** | `results/prueba_carga.txt` | Memoria 86 %, CPU 100 % |
...
| RF-4 | Menú de navegación entre las tres funciones | `io/Menu` | `results/vistas_menu.txt`; `EJECUTAR.bat` | Cumple |
...
**Resultado actual: 26 de 26 pruebas superadas en ambas plataformas.**
...
| **Resultado** | Memoria al 86,4 %, CPU al 100 % al empezar y al terminar, 13 de 13 trabajos vivos |
...
| Métrica | Valor |
|---|---:|
| Pruebas automatizadas | 26 |
| Pruebas superadas en Windows | 26 |
| Pruebas superadas en Linux | 26 |
| Plataformas verificadas | 2 |
| Defectos detectados y cerrados | 9 |
| Defectos abiertos | 0 |
| Memoria residente de la herramienta | 4,0 MB |
| Ralentización media bajo carga | 2,13 (entre 1,59 y 3,83) |
...
| 1 | Ejecuta la suite | `26/26 pruebas superadas` |
...
resultados consistentes en ejecuciones repetidas». 26 pruebas que pasan en dos
...
advertencias y pasa las 26 pruebas en Windows con MSVC y en Linux con g++.
...
8. **La memoria por proceso incluye las páginas compartidas.** Es el *working
   ...
   «memoria sumada de todos» de la lista de procesos cuenta varias veces las
   bibliotecas compartidas y no equivale a la memoria usada del sistema, que es
   la que muestra la opción 3 del menú. No se cambió la salida del programa
   para no invalidar la evidencia ya generada; queda declarado aquí y en la
   sección 8 del documento IEEE.
...
Con resultado: **26/26 pruebas superadas en Windows**, **26/26 en Linux**,
**cero advertencias en ambos compiladores**, **`windows.h` confinado en
`src/plataforma/`**, **carga del RE-2 comprobada (memoria 86,4 %, CPU
